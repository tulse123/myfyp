...
from .models import Patient, Services, Chatpatient
...
def afterlogin_view(request):
    if is_admin(request.user):
        ...
        accountapproval=models.Receptionist.objects.all().filter(user_id=request.user.id, status=False)
        if accountapproval:
...
#allow receptionist to view patient details
@user_passes_test(is_receptionist)
...
#view appointment by receptionist
@user_passes_test(is_receptionist)
...
def aboutus_view(request):
    ...
    return render(request,'hospital/aboutus.html', {'services': services})
def all_services(request):
    services=Services.objects.all()
    if request.method == 'POST':
        heading=request.POST.get('heading')
        paragraph=request.POST.get('paragraph')
        obj=Services.objects.create(heading=heading, paragraph=paragraph)
        obj.save()
        print("donee")
        messages.success(request, "Added successfull")
        return HttpResponseRedirect('/all_services')
    else:
        return render(request, 'hospital/all_services.html', {'services': services})
def show_services(request):
    services=Services.objects.all()
    return render(request, "hospital/show_services.html", {'services': services})
def edit_services(request, id):
    if request.method == 'POST':
        heading=request.POST.get('heading')
        paragraph=request.POST.get('paragraph')
        find_id=Services.objects.get(id=id)
        find_id.heading=heading
        find_id.paragraph=paragraph
        find_id.save()
        messages.success(request, "edit successfull")
        return HttpResponseRedirect("/all_services")
    else:
        return render(request, "hospital/edit_services.html")
def delete_services(request, id):
    obj=Services.objects.get(id=id)
    obj.delete()
    return HttpResponseRedirect("/show_services")
#message send by patient
def message_send(request):
    if request.method != 'POST':
        patient=models.Patient.objects.get(user_id=request.user.id)
        data=Chatpatient.objects.filter(patient_id=patient)
        return render(request, "hospital/patient_chat.html", {'patient':patient, 'data': data})
    else:
        patient=Patient.objects.get(user=request.user.id)
        message=request.POST.get('message')
        obj=Chatpatient.objects.create(patient_id=patient, feedback=message, feedback_reply="")
        obj.save()
...
def chat_reply(request):
    if request.method != 'POST':
        # patient=Patient.objects.get(user=request.user.id)
        data=Chatpatient.objects.all()
        return render(request, "hospital/chat_reply.html", {'data': data})
    else:
        return HttpResponse('method not valid')
@csrf_exempt
def chat_replied(request):
    if request.method == 'POST':
        feedback_id=request.POST.get('id')
        feedback_message=request.POST.get('message')
        try: 
            feedback=Chatpatient.objects.get(id=feedback_id)
